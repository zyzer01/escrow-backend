...
export class BetDisputeService {

    public async logDispute(betId: string, userId: string, reason: string): Promise<void> {
        ...
        await createNotification(
            ...
            `A dispute has been logged against your bet - '${bet.title}'`
        );
        await createNotification(
            bet.opponentId,
            "bet-dispute",
    ...
    public async resolveDispute(disputeId: string, resolution: 'resolved' | 'rejected', action: 'refund' | 'reverse' | 'confirm'): Promise<void> {
    ...
    public async getAllDisputes(): Promise<IBetDispute[]> {
        return BetDispute.find().populate('betId').populate('reportedBy').sort({ createdAt: -1 });
    }

    public async getDisputes(userId: string): Promise<IBetDispute[]> {
        return BetDispute.find({ userId }).populate('betId').populate('reportedBy').sort({ createdAt: -1 });
    }

    public async getDispute(userId: string): Promise<IBetDispute[]> {
        return BetDispute.findOne({ userId }).populate('betId').populate('reportedBy').sort({ createdAt: -1 });
    }
}


export const betDisputeService = new BetDisputeService();
